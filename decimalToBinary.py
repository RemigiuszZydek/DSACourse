...
class LinkedList:
    ...
    def binary_to_decimal(self):
        """
        function that converts binary linked list into decimal number. 
        Function iterates through linked list and for each node it multiplies current value by 2 and adds a node value.
        I did't add if statement to check if node value is 0 beacause it will add 0 anyaway so my value wont change."""
        value = 0
        current = self.head
        if current is None:
            return False
